...
fun selectDependencies() = when(publicationName) {
    ...
    "generator" -> generatorDependencies.externalDependencies() + listOf(Dependency(group.toString(), "kotlin-test-factories-annotations", version.toString()))
    else -> emptyList()
}
...
data class Dependency(val group: String, val artifactId: String, val version: String) {
    companion object {
        fun from(dependency: GradleDependency): Dependency? {
            ...
            val version = dependency.group ?: return null
            return Dependency(
                    group,
                    dependency.name,
                    version
